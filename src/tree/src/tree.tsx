...
export default defineComponent({
  ...
  setup(props: TreeProps, { slots }) {
    ...
    const { expandedTree, toggleNode, getExpandedChildren, toggleCheckNode, toggleInCheckNode } = useTree(data)
      ...
      return <div class="s-tree">
        {
          expandedTree.value.map(treeNode => {
            return <div class="s-tree-node hover:bg-slate-100 relative leading-7" style={{paddingLeft: `${NODE_INDENT * (treeNode.level - 1)}px`}}>
              ...
              {/* 复选框 */}
              { checkable.value && 
                <span class={`s-tree-checkbox relative ${treeNode.inChecked ? 's-tree__inChecked' : ''}`}>
                  {treeNode.inChecked && <span class="s-tree-checkbox__inner cursor-pointer" onClick={() => toggleInCheckNode(treeNode)}>-</span>}
                  <input type="checkbox" class="cursor-pointer ml-[2px] mr-[2px]" v-model={treeNode.checked} onChange={() => toggleCheckNode(treeNode)} />
                </span>
              }
              {/* 标签 */}
