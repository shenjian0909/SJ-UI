...
export function generateTreeData(data: ITreeNode[], level = 0, path = [] as IInnerTreeNode []): IInnerTreeNode[] {
  ...
  return data.reduce((prev, cur) => {
    let tempNode = {...cur} as IInnerTreeNode
    ...
    if(tempNode.children) {
      let children = generateTreeData(tempNode.children, level, path)
